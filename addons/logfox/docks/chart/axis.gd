...
func _draw() -> void:
	if vert_grad.is_empty() or hor_grad.is_empty():
		return
	...
	if show_x_ticks:
		for grad in hor_grad:
			draw_line(grad[POINT], grad[POINT] + Vector2(0, 10), Color.hex(0x94E2D5ff))
	...
	if show_x_numbers:
		for grad in hor_grad:
			draw_string(
				...
				-1,
				16,
				Color.hex(0x94E2D5ff)
			)

	if show_horizontal_line == true:
		draw_line(hor_grad.front()[POINT], hor_grad.back()[POINT], Color.hex(0x94E2D5ff))

	if show_y_ticks == true:
		for grad in vert_grad:
			draw_line(grad[POINT], grad[POINT] - Vector2(10, 0), Color.hex(0x94E2D5ff))

	if show_y_numbers == true:
		for grad in vert_grad:
			draw_string(
				default_font, grad[0] + Vector2(-35, -5), grad[1], 0, -1, 16, Color.hex(0x94E2D5ff)
			)

	if show_vertical_line == true:
		draw_line(topleft, vert_grad.back()[POINT], Color.hex(0x94E2D5ff))
