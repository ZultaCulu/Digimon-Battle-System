...
const GRAPH_2D_AXIS = preload("res://addons/logfox/docks/chart/axis.gd")
const GRAPH_2D_COORD = preload("res://addons/logfox/docks/chart/coordinate.gd")
const GRAPH_2D_GRID = preload("res://addons/logfox/docks/chart/grid.gd")
const GRAPH_2D_LEGEND = preload("res://addons/logfox/docks/chart/legend.gd")
...
func _ready():
	...
	var axis = GRAPH_2D_AXIS.new()
	add_child(axis)

	var grid = GRAPH_2D_GRID.new()
	add_child(grid)

	var legend = GRAPH_2D_LEGEND.new()
	plot_area.add_child(legend)

	var coordinate = GRAPH_2D_COORD.new()
...
func add_plot_item(label = "", color = Color.hex(0x94E2D5ff), width = 1.0) -> PlotItem:
...
func _update_graph() -> void:
	if get_node_or_null("Axis") == null:
		return
	if get_node_or_null("Grid") == null:
		return
	if get_node_or_null("PlotArea") == null:
		return
	...
	var margin_bottom: float = (
		_MARGIN_BOTTOM if get_node("Axis").x_label == "" else _MARGIN_BOTTOM + 20
	)
